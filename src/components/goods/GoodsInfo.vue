...
export default {
    ...
    methods: {
        getLunbotu() {
            ...
            //         // 先循环轮播图数组的每一项，为 item 添加 img 属性，因为 轮播图组件中，只认识 item.img ， 不认识 item.src
            ...
            this.$http.get('http://localhost:3001/getgoodslist?pageindex=' + this.pageindex ).then(result => {
                ...
                if(result.status === 200) {
                    this.lunbotu = result.body.BAI6I0O5wangning;
                }
            })
        },
        getGoodsInfo() {
        ...
        enter(el, done) {
            ...
            // 小球动画优化思路：
            // 1. 先分析导致 动画 不准确的 本质原因： 我们把 小球 最终 位移到的位置，已经局限住在了某一分辨率下的 滚动条未滚动的情况下；
            // 2. 只要分辨率和 测试的时候不一样，或者 滚动条 有一定的滚动距离之后，问题就出现了；
            // 3. 因此，我们经过分析，得到结论： 不能把 位置的 横纵坐标 直接写死了，而是 应该 根据不同情况，动态计算这个坐标值
            // 4. 经过分析，得出解题思路： 先得到 徽标的 横纵坐标，在得到 小球的 横纵坐标，然后 让 y 值和 x 值求差 得到的结果，就是横纵坐标的距离
